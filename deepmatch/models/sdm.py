...
def SDM(user_feature_columns, history_feature_list, item_feature_columns, units=64, rnn_layers=2, dropout_rate=0.2, grad_norm=5,
        rnn_num_res=1, num_head=4, l2_reg_embedding=1e-6, dnn_activation='tanh', init_std=0.0001, seed=1024,
        num_sampled=5):
    ...
    target_columns = [item_feature_name]
    ...
    feature_len_list = [features[i+'_len'] for i in history_feature_list]

    item_inputs_list = list(item_features.values())

    prefer_att_outputs = []
    for i, prefer_emb in enumerate(prefer_emb_list):
        feature_len = feature_len_list[i]
        prefer_attention_output = AttentionSequencePoolingLayer(dropout_rate=0)([user_emb, prefer_emb, feature_len])
    ...
    short_rnn_output = DynamicMultiRNN(units, return_sequence=True, num_layers=rnn_layers, num_residual_layers=rnn_num_res,
                                       dropout_rate=dropout_rate)([short_emb_list, user_sess_length])

    short_att_output = SelfMultiHeadAttention(num_units=units, head_num=num_head, dropout_rate=dropout_rate, future_binding=True,
                                              use_layer_norm=True)([short_rnn_output, user_sess_length]) # [batch_size, time, num_units]

    short_output = UserAttention(num_units=units, activation=dnn_activation, use_res=True, dropout_rate=dropout_rate)\
        ([user_emb, short_att_output, user_sess_length])

    # gated combine
    gate_input = concat_func([prefer_output, short_output, user_emb])
    gate = Dense(units, activation='sigmoid')(gate_input)
    gate_output = Lambda(lambda x, y, z: tf.multiply(x, y) + tf.multiply(1-x, z))(gate, short_output, prefer_output)
    ...
    output = SampledSoftmaxLayer(num_sampled=num_sampled)(
        inputs=(pooling_item_embedding_weight, gate_output, item_features[item_feature_name]))
