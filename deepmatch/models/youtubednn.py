...
def YoutubeDNN(user_feature_columns, item_feature_columns, num_sampled=5,
    ...
    item_embedding = embedding_matrix_dict[item_feature_name]#Lambda( lambda x:embedding_matrix_dict[item_feature_name])(None)
    # if not item_embedding.built:
    #     item_embedding.build()
    item_embedding = Input(tensor=item_embedding.embeddings,name=item_feature_name+"_embeddings")
    ...
    model = Model(inputs=user_inputs_list + item_inputs_list+[item_embedding], outputs=output)#+[item_idx]
